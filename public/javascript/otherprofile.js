...
$.getJSON(window.location.origin + '/other-profile/getDataOtherprof/' + segmentTerakhir, function(res) {
    console.log(res);
    $('#namauser').html(res.dataUser.user.user_name);
    $('#bio').html(res.dataUser.user.bio);
    $('#notel').html(res.dataUser.user.no_telp);
    $('#nama').html(res.dataUser.user.name);
    $('#imageuser').prop('src', '/foto/' + res.dataUser.user.picture);
